...
class LabEquipmentIntraDomainReclassifier:
    """Reclassify Lab Equipment products from generic to specific subcategories - OPTIMIZED FOR SPEED"""

    def __init__(self):
        self.yaml_structure = LabEquipmentYAMLStructure()
        self.search_cache = {}
        self.pattern_cache = {}  # NEW: Cache by product type patterns
        self.total_searches = 0
        self.successful_searches = 0
        self.skipped_searches = 0  # NEW: Track skipped searches
        self.reclassification_stats = defaultdict(int)

        # SPEED OPTIMIZATION: Pre-compile obvious product patterns
        self.obvious_patterns = {
            "spectrophotometer": ("Spectroscopy", "UV-Vis Spectrophotometers"),
            "spectrometer": ("Spectroscopy", "UV-Vis Spectrophotometers"),
            "flow cytometer": ("Cell Analysis", "Flow Cytometers"),
            "cytometer": ("Cell Analysis", "Flow Cytometers"),
            "centrifuge": ("General Laboratory Equipment", "Centrifuges"),
            "ultracentrifuge": ("General Laboratory Equipment", "Centrifuges"),
            "microcentrifuge": ("General Laboratory Equipment", "Centrifuges"),
            ...
            "mass spectrometer": ("Mass Spectrometry", "Mass Spectrometers"),
            "qpcr": ("PCR Equipment", "PCR Thermocyclers"),
            "real-time pcr": ("PCR Equipment", "PCR Thermocyclers"),
            "thermocycler": ("PCR Equipment", "PCR Thermocyclers"),
            "microplate reader": ("Spectroscopy", "Microplate Readers"),
            "plate reader": ("Spectroscopy", "Microplate Readers"),
            "whatman filter": ("Laboratory Supplies & Consumables", "Filters"),
            "filter paper": ("Laboratory Supplies & Consumables", "Filters"),
            "membrane filter": ("Laboratory Supplies & Consumables", "Filters"),
            "pcr plate": ("Laboratory Supplies & Consumables", "PCR Consumables"),
            "96-well plate": (
    ...
    def check_obvious_classification(
        ...
        for pattern, (subcategory, subsubcategory) in self.obvious_patterns.items():
            if pattern in product_lower:
                logger.info(
                    f"🚀 OBVIOUS PATTERN: '{product_name}' → {subcategory} → {subsubcategory}"
                )
                self.skipped_searches += 1
                return (subcategory, subsubcategory, "High")
    ...
    def get_optimized_search_description(
        ...
        if obvious:
            subcategory, subsubcategory, confidence = obvious
            return f"Laboratory equipment: {product_name}. Type: {subcategory}. Specific category: {subsubcategory}. Manufacturer: {manufacturer}"

        # If not obvious, create basic description for classification
        return f"Laboratory equipment product: {product_name} by {manufacturer}. Requires classification within Lab Equipment domain."
    ...
    def reclassify_lab_equipment_product(
        ...
        """OPTIMIZED: Reclassify a single Lab Equipment product to specific subcategory"""
        ...
        if obvious_classification:
            subcategory, subsubcategory, confidence = obvious_classification
            ...
            classification_result = {
                ...
                "path_components": [subcategory, subsubcategory],
                "is_valid_path": True,
                "validated_path": f"Lab Equipment -> {subcategory} -> {subsubcategory}",
                "depth_achieved": 2,
                "token_usage": 0,
            }
...
{{
    ...
    "search_successful": true
        ...
        try:
            ...
            time.sleep(0.05)  # SPEED OPTIMIZATION: Reduced from 0.2s to 0.05s
            return result
...
def process_lab_equipment_reclassification_with_checkpointing(
    ...
    try:
        ...
        progress_desc = f"🚀 OPTIMIZED Lab Equipment Reclassification ({already_processed_count} done)"
        ...
        progress_bar = tqdm(
            ...
            total=len(target_df),
            initial=0,
            miniters=10,  # Update every 10 items instead of every item
        )
        ...
        logger.info(
            f"🚀 SPEED OPTIMIZATIONS APPLIED - Estimated 60-80% faster than original"
        )
...
def generate_lab_equipment_reclassification_report(
    ...
    """Generate OPTIMIZED report for Lab Equipment reclassification results"""

    print(f"\n{'='*80}")
    print("🔧 LAB EQUIPMENT INTRA-DOMAIN RECLASSIFICATION REPORT (OPTIMIZED)")
    print(f"{'='*80}")
    ...
    print(f"\n{'SPEED OPTIMIZATIONS APPLIED':-^60}")
    print(f"  ✅ Obvious pattern detection (skips web search)")
    print(f"  ✅ Aggressive pattern caching")
    print(f"  ✅ Reduced API sleep time (0.05s vs 0.2s)")
    print(f"  ✅ Optimized YAML prompt structure")
    print(f"  ✅ Smart search skipping for clear products")


def main():
    """Main function for OPTIMIZED Lab Equipment intra-domain reclassification"""
    ...
    try:
        ...
        if choice == "1":
            print(
                "\n Starting OPTIMIZED Lab Equipment intra-domain reclassification..."
            )

            user_confirmation = input(
                "  This will process Lab Equipment products. Continue? (y/n): "
            )
            ...
            print("\n🔄 Resuming from latest Lab Equipment checkpoint (OPTIMIZED)...")
            latest_checkpoint, checkpoint_number = (
                find_latest_lab_equipment_checkpoint()
            )
            if latest_checkpoint:
