...
class WebSearchProductClassifier:
    ...
    def search_product_info(
        ...
        """IMPROVED: Use domain-guided web search instead of generic search"""

        # Create search query (keep existing logic)
        search_query = product_name.strip()
        if manufacturer and manufacturer.strip():
            search_query = f"{product_name.strip()} {manufacturer.strip()}"

        # Clean up search query (keep existing logic)
        search_query = re.sub(r"[^\w\s-]", " ", search_query)
        search_query = " ".join(search_query.split())

        # Check cache first (keep existing logic)
            ...
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": self._get_improved_search_prompt(
                ...
                logger.warning(
                    f"⚠️ Search attempt {attempt + 1} failed for '{search_query}': {e}"
                )
                if attempt < MAX_RETRIES - 1:
                    time.sleep(2**attempt)

        # If all attempts failed (keep existing logic)
    ...
    def _get_improved_search_prompt(self, product_name: str, manufacturer: str) -> str:
        """Domain-aware search prompt using master categories knowledge"""

        # Get domain knowledge from  master config
        available_domains = self.category_system.available_domains
        domain_mapping = self.category_system.master_config.get("domain_mapping", {})

        # Build domain context
        domain_hints = []
        key_indicators = []

        for domain_key in available_domains[:8]:  # Top 8 domains for brevity
            domain_info = domain_mapping.get(domain_key, {})
            domain_name = domain_key.replace("_", " ")
            description = domain_info.get("description", "")[
                :100
            ]  # Truncate for prompt size
            keywords = domain_info.get("keywords", [])[:3]  # Top 3 keywords

            if description:
                domain_hints.append(f"• {domain_name}: {description}")
            key_indicators.extend(keywords)

        return f"""You are a life science product specialist researching: "{product_name}" by {manufacturer}

    LIFE SCIENCE DOMAINS TO CONSIDER:
    {chr(10).join(domain_hints)}

    KEY CLASSIFICATION INDICATORS: {', '.join(key_indicators[:15])}

    FIND SPECIFIC INFORMATION ABOUT:

    🎯 **PRIMARY TECHNIQUE**: What lab technique is this used for?
    Examples: PCR/qPCR, Western blot, flow cytometry, ELISA, cell culture, chromatography, microscopy, immunohistochemistry, DNA sequencing

    🔬 **PRODUCT TYPE**:
    - Instrument/Equipment: Physical devices with electronics (microscopes, centrifuges, PCR machines, readers)
    - Reagent/Chemical: Chemical compounds, antibodies, enzymes, buffers, stains, media
    - Kit/Assay: Complete test systems (ELISA kits, extraction kits, detection kits)  
    - Consumable: Disposable items (tubes, plates, membranes, tips, columns)

    🧬 **RESEARCH APPLICATION**: 
    - What research areas use this? (cell biology, molecular biology, protein analysis, immunology)
    - What workflow step? (sample prep, analysis, detection, purification, culture)

    ⚙️ **TECHNICAL DETAILS**:
    - Performance specs, compatibility, sample types
    - How it fits into research workflows

    SEARCH STRATEGY:
    1. Check manufacturer website ({manufacturer}.com if available)
    2. Look at major suppliers (Thermo Fisher, Bio-Rad, Sigma-Aldrich catalogs)
    3. Find application notes and protocol references

    CLASSIFICATION HINTS:
    - Antibody products → Look for "anti-", target proteins, applications (Western, IHC, Flow)
    - Equipment → Look for "system", "analyzer", "reader", technical specifications
    - Cell biology → Look for "cell culture", "staining", "viability", flow cytometry use
    - PCR → Look for "amplification", "polymerase", "real-time", thermal cycling
    - Protein → Look for "Western blot", "purification", "chromatography", protein transfer

    Provide detailed technical information focusing on the product's specific laboratory applications and research use."""

    def extract_enhanced_product_info(
        self, search_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Extract domain-specific information instead of generic categories"""
        ...
        extraction_prompt = f"""Extract structured information from this life science product search:

    {product_info}

    AVAILABLE LIFE SCIENCE DOMAINS: {', '.join(self.category_system.available_domains)}

    Extract JSON format:
    {{
        "primary_technique": "specific technique (PCR, Western_blot, flow_cytometry, ELISA, etc.)",
        "product_type": "instrument, reagent, kit, or consumable",
        "research_applications": ["cell_biology", "molecular_biology", "protein_analysis", "etc"],
        "workflow_position": "sample_prep, analysis, detection, purification, or culture",
        "technical_details": "specific features and capabilities",
        "suggested_domains": ["most_likely_domain_from_available_list"],
        "confidence": "High, Medium, or Low",
        "enhanced_description": "comprehensive description combining all findings"
    }}

    DOMAIN MAPPING RULES:
    - PCR/qPCR → "PCR" domain
    - Western blot/protein transfer → "Protein" domain  
    - Flow cytometry/cell analysis → "Cell_Biology" domain
    - ELISA/immunoassays → "Assay_Kits" domain
    - Antibodies → "Antibodies" domain
    - Physical instruments → "Lab_Equipment" domain
    - Chemical reagents → "Chemistry" domain
    - Microscopy → "Bioimaging_Microscopy" domain

    Use EXACT domain names from the available domains list above.
    ...
    Respond with JSON only:"""
        ...
        try:
            ...
            # Validate suggested domains
            suggested_domains = extraction_result.get("suggested_domains", [])
            validated_domains = []

            for domain in suggested_domains:
                if domain in self.category_system.available_domains:
                    validated_domains.append(domain)

            extraction_result["validated_domains"] = validated_domains
            ...
            logger.info(
                f"✅ Extracted technique: {extraction_result.get('primary_technique', 'Unknown')}"
            )
            logger.info(f"✅ Suggested domains: {validated_domains}")
            ...
            return {
                "enhanced_description": product_info,
                "product_type": "Unknown",
                "primary_application": "Unknown",
                "research_areas": [],
                "technical_details": "",
    ...
    def reclassify_with_web_info(
        self, product_name: str, manufacturer: str, original_description: str = ""
    ) -> Dict[str, Any]:
        ...
        if not search_result.get("search_successful", False):
            logger.warning(
                f"⚠️ Web search failed for '{product_name}', using original classification"
            )
            return self._create_failed_search_result(
                product_name, original_description, total_tokens
            )
        ...
        if not enhanced_info.get("extraction_successful", False):
            ...
            return self._create_failed_extraction_result(
                product_name, search_result, total_tokens
            )
        ...
        combined_description = self._create_combined_description(
            original_description, enhanced_info.get("enhanced_description", "")
        )
        ...
        # Update stats
        new_domain = classification_result.get("primary_classification", {}).get(
            "domain", "Other"
        )
    ...
    def _create_combined_description(
        self, original: str, enhanced: str, enhanced_info: Dict = None
    ) -> str:
        """Create domain-intelligent combined description"""
        ...
        if enhanced_info:
            technique = enhanced_info.get("primary_technique", "")
            if technique:
                parts.append(f"Primary Technique: {technique}")

            workflow = enhanced_info.get("workflow_position", "")
            if workflow:
                parts.append(f"Workflow: {workflow}")

            suggested_domains = enhanced_info.get("validated_domains", [])
            if suggested_domains:
                parts.append(f"Suggested Domains: {', '.join(suggested_domains)}")

        return " | ".join(parts) if parts else ""

    def _create_failed_search_result(
        self, product_name: str, description: str, tokens: int
    ) -> Dict[str, Any]:
    ...
    def _create_failed_extraction_result(
        self, product_name: str, search_result: Dict, tokens: int
    ) -> Dict[str, Any]:
...
def load_and_filter_other_products(csv_file: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"🎯 Found {len(other_products)} products classified as 'Other' ({len(other_products)/len(df)*100:.1f}%)"
        )
...
def process_web_search_reclassification(
    ...
    web_classifier = WebSearchProductClassifier(
        category_system, tag_system, base_classifier
    )
    ...
    logger.info(
        f"🔄 Processing {len(other_products_df)} products with web search reclassification..."
    )
    ...
    for idx in tqdm(other_products_df.index, desc="🌐 Web Search Reclassification"):
        ...
        manufacturer = (
            other_products_df.at[idx, "Manufacturer"]
            if "Manufacturer" in other_products_df.columns
            else ""
        )
        original_description = (
            other_products_df.at[idx, "Description"]
            if "Description" in other_products_df.columns
            else ""
        )
        ...
        try:
            ...
            result = web_classifier.reclassify_with_web_info(
                name, manufacturer, original_description
            )

            # Store web search metadata
            other_products_df.at[idx, "web_search_performed"] = result.get(
                "web_search_performed", False
            )
            other_products_df.at[idx, "web_search_successful"] = result.get(
                "web_search_successful", False
            )
            other_products_df.at[idx, "search_query"] = result.get("search_query", "")
            other_products_df.at[idx, "reclassification_confidence"] = result.get(
                "reclassification_confidence", "Low"
            )

            # Store enhanced product info
            enhanced_info = result.get("enhanced_product_info", {})
            other_products_df.at[idx, "enhanced_product_type"] = enhanced_info.get(
                "product_type", ""
            )
            other_products_df.at[idx, "enhanced_primary_application"] = (
                enhanced_info.get("primary_application", "")
            )
            other_products_df.at[idx, "enhanced_research_areas"] = "|".join(
                enhanced_info.get("research_areas", [])
            )
            other_products_df.at[idx, "enhanced_description_full"] = enhanced_info.get(
                "enhanced_description", ""
            )
            ...
            if classifications:
                ...
                other_products_df.at[idx, "new_primary_domain"] = primary.get(
                    "domain", ""
                )
                other_products_df.at[idx, "new_primary_subcategory"] = primary.get(
                    "subcategory", ""
                )
                other_products_df.at[idx, "new_primary_subsubcategory"] = primary.get(
                    "subsubcategory", ""
                )
                other_products_df.at[idx, "new_primary_subsubsubcategory"] = (
                    primary.get("subsubsubcategory", "")
                )
                other_products_df.at[idx, "new_primary_confidence"] = primary.get(
                    "confidence", ""
                )
                other_products_df.at[idx, "new_primary_fit_score"] = primary.get(
                    "domain_fit_score", 0
                )
                other_products_df.at[idx, "new_validated_path"] = primary.get(
                    "validated_path", ""
                )

                # Store tags
                other_products_df.at[idx, "new_technique_tags"] = "|".join(
                    primary.get("technique_tags", [])
                )
                other_products_df.at[idx, "new_research_tags"] = "|".join(
                    primary.get("research_tags", [])
                )
                other_products_df.at[idx, "new_functional_tags"] = "|".join(
                    primary.get("functional_tags", [])
                )
                other_products_df.at[idx, "new_total_tags"] = primary.get(
                    "total_tags", 0
                )

            # Store token usage
            other_products_df.at[idx, "web_search_tokens"] = result.get(
                "web_search_tokens", 0
            )
            other_products_df.at[idx, "extraction_tokens"] = result.get(
                "extraction_tokens", 0
            )
            other_products_df.at[idx, "reclassification_tokens"] = result.get(
                "total_token_usage", 0
            )
            other_products_df.at[idx, "total_web_enhanced_tokens"] = result.get(
                "total_web_enhanced_tokens", 0
            )
    ...
    reclassification_details_csv = output_csv.replace(
        ".csv", "_reclassification_details.csv"
    )
    ...
    logger.info(
        f"📈 Success rate: {successful_reclassifications/len(other_products_df)*100:.1f}%"
    )
...
def merge_reclassified_results(
    complete_df: pd.DataFrame, reclassified_df: pd.DataFrame
) -> pd.DataFrame:
    """Merge reclassified 'Other' products back into the complete dataset"""

    logger.info(
        f"🔄 Merging {len(reclassified_df)} reclassified products into {len(complete_df)} total products"
    )
    ...
    for idx, reclassified_row in reclassified_df.iterrows():
        ...
        if idx in updated_df.index:
            ...
            if reclassified_row.get(
                "web_search_successful", False
            ) and reclassified_row.get("new_primary_domain", "") not in ["", "Other"]:

                # Update primary classification columns with new results
                updated_df.at[idx, "primary_domain"] = reclassified_row.get(
                    "new_primary_domain", ""
                )
                updated_df.at[idx, "primary_subcategory"] = reclassified_row.get(
                    "new_primary_subcategory", ""
                )
                updated_df.at[idx, "primary_subsubcategory"] = reclassified_row.get(
                    "new_primary_subsubcategory", ""
                )
                updated_df.at[idx, "primary_subsubsubcategory"] = reclassified_row.get(
                    "new_primary_subsubsubcategory", ""
                )
                updated_df.at[idx, "primary_confidence"] = reclassified_row.get(
                    "new_primary_confidence", ""
                )
                updated_df.at[idx, "primary_fit_score"] = reclassified_row.get(
                    "new_primary_fit_score", 0
                )
                updated_df.at[idx, "validated_path_primary"] = reclassified_row.get(
                    "new_validated_path", ""
                )

                # Update tags
                updated_df.at[idx, "technique_tags"] = reclassified_row.get(
                    "new_technique_tags", ""
                )
                updated_df.at[idx, "research_tags"] = reclassified_row.get(
                    "new_research_tags", ""
                )
                updated_df.at[idx, "functional_tags"] = reclassified_row.get(
                    "new_functional_tags", ""
                )
                updated_df.at[idx, "total_tags"] = reclassified_row.get(
                    "new_total_tags", 0
                )
                ...
                logger.info(
                    f"✅ Updated {updated_df.at[idx, 'Name'][:30]} → {reclassified_row.get('new_primary_domain', '')}"
                )
...
def generate_web_search_reclassification_report(
    df: pd.DataFrame, classifier: WebSearchProductClassifier
):
    ...
    successful_reclassifications = len(
        df[
    ...
    print(
        f"  Successful web searches: {successful_searches} ({successful_searches/total_products*100:.1f}%)"
    )
    print(
        f"  Successful reclassifications: {successful_reclassifications} ({successful_reclassifications/total_products*100:.1f}%)"
    )

    # New domain distribution
    print(f"\n{'NEW DOMAIN DISTRIBUTION':-^60}")
    new_domain_counts = df[df["new_primary_domain"] != ""][
        "new_primary_domain"
    ].value_counts()
    ...
    product_type_counts = df[df["enhanced_product_type"] != ""][
        "enhanced_product_type"
    ].value_counts()
...
def test_web_search_reclassification():
    ...
    for i, test_product in enumerate(test_products, 1):
        ...
        try:
            result = web_classifier.reclassify_with_web_info(
                test_product["name"], test_product["manufacturer"]
            )
    ...
    print(f"\n{'='*80}")
    print(
        f"🧪 TEST RESULTS: {success_count}/{len(test_products)} passed ({success_count/len(test_products)*100:.1f}%)"
    )
    print(f"{'='*80}")
...
def main():
    ...
    try:
        ...
        if test_success:
            print("\n" + "=" * 80)
            user_input = input(
                "✅ Tests passed! Run reclassification on validation data? (y/n): "
            )
