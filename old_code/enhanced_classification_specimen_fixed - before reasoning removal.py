...
VALIDATION_CSV = (
    "validation_enhanced_fixed_classification_w_tagging_prompt_changes_4.csv"
)
            ...
            classification.update(
                {
                    ...
                    "specimen_tags": tagging_result.get(
                        "specimen_tags", []
                    ),  # ADD SPECIMEN
...
def get_enhanced_csv_columns():
    """Complete column structure with all missing fields"""
    return [
        ...
        "primary_fit_score",
        "primary_path_valid",
        "primary_depth_achieved",  # ADD: 1, 2, 3, or 4
        ...
        "secondary_depth_achieved",  # ADD: 1, 2, 3, or 4
        "validated_path_secondary",
        # Dual classification metadata
        "is_dual_function",
        "dual_function_reasoning",
        ...
        "domain_selection_tokens",  # ADD: Stage 1 token usage
        # Tagging results - ALL 4 CATEGORIES
        "technique_tags",
        "research_tags",
        "functional_tags",
        "specimen_tags",  # ADD: Missing specimen tags
        "all_tags",  # Combined unique tags
        "total_tags",
        "tag_confidence",
        "tagging_reasoning",
        # Token usage breakdown
        "primary_path_tokens",  # ADD: Stage 2a tokens
        "secondary_path_tokens",  # ADD: Stage 2b tokens
        "tagging_tokens",  # ADD: Stage 3 tokens
        "total_token_usage",  # Total across all stages
        # Web search enhancement (for reclassification)
        "web_search_attempted",  # ADD: Bool - even for non-Other
        ...
        # System metadata
        "classification_timestamp",  # ADD: ISO timestamp
        "processing_time_seconds",  # ADD: Performance tracking
        "system_version",  # ADD: "v2.0_enhanced_dual"
        "chain_prompting_stages",  # 3 for normal, more for web search
        "error_occurred",  # ADD: Bool for error tracking
        "error_message",  # ADD: Error details if any
    ]
...
def process_enhanced_validation_sample():
    """FIXED: Enhanced validation with all columns including specimen tags"""
    ...
    try:
        ...
        for col in new_columns:
            if col in [
                ...
                "domain_selection_tokens",
                "primary_path_tokens",
                "secondary_path_tokens",
                "tagging_tokens",
        ...
        for idx in tqdm(validation_df.index, desc="🔄 Enhanced Dual Classification"):
            ...
            try:
                ...
                validation_df.at[idx, "classification_timestamp"] = timestamp
                validation_df.at[idx, "processing_time_seconds"] = processing_time
                validation_df.at[idx, "system_version"] = "v2.0_enhanced_dual_tagging"
                validation_df.at[idx, "web_search_attempted"] = (
                    False  # For regular classification
                )
                ...
                validation_df.at[idx, "error_message"] = str(e)
...
def main():
    """STREAMLINED: Production-ready main function with testing removed"""
    print("=" * 80)
    print("🚀 ENHANCED CLASSIFICATION SYSTEM - PRODUCTION MODE")
    print("   Chain Prompting: Domain → Path → Tags")
    print("   Features: Dual Classification + Specimen Tags")
    print("=" * 80)

    try:
        print("\n📊 Starting validation sample processing...")
        ...
        if len(validation_df) > 0:
            success_rate = (
                len(
                    validation_df[
                        (validation_df["primary_domain"] != "Other")
                        & (validation_df["primary_domain"] != "")
                        & (validation_df["error_occurred"] == False)
                    ]
                )
                / len(validation_df)
                * 100
            ...
            print(f"   • Total Products: {len(validation_df)}")
            print(f"   • Average Tags: {validation_df['total_tags'].mean():.1f}")
            print(
                f"   • Specimen Tags Working: {'✅' if validation_df['specimen_tags'].str.len().sum() > 0 else '❌'}"
            )
            print(
                f"   • Dual Classifications: {len(validation_df[validation_df['is_dual_function'] == True])}"
            )

            full_input = input(
                "\n🚀 Validation successful! Proceed with full dataset? (y/n): "
            )
            if full_input.lower() == "y":
                print("🔄 Starting full dataset processing...")
                # process_full_dataset()  # You'll need to implement this
                print(
                    "📝 Note: Implement process_full_dataset() function for full processing"
                )
            else:
                print("✅ Processing stopped at validation stage.")
        else:
            print("❌ Validation failed - no products processed successfully.")
