...
VALIDATION_CSV = "validation_llm_driven_classification_unlimited_claude_changes.csv"
...
class LLMDrivenCategorySystem:
    """Enhanced LLM-driven category system with comprehensive YAML context"""
...
class LLMDrivenClassifier:
    """Enhanced LLM-driven classifier with comprehensive YAML context support"""
...
{{
    ...
    def _classify_within_domain(
        ...
        """Enhanced within-domain classification with comprehensive YAML context and deep path enforcement"""
    ...
    {{
        ...
        try:
            ...
            self.logger.info(
                f"COMPREHENSIVE Classification depth achieved for '{product_name}': {depth_achieved} levels"
            )

            if depth_achieved < 2:
                self.logger.warning(
                    f"Shallow classification for '{product_name}': only {depth_achieved} levels despite comprehensive context"
                )
            elif depth_achieved >= 3:
                self.logger.info(
                    f"✅ Good depth achieved for '{product_name}': {depth_achieved} levels using comprehensive context"
                )
...
def process_validation_sample():
    ...
    try:
        ...
        logger.info(
            f"Processing {len(validation_df)} products with comprehensive YAML context..."
        )
...
def generate_validation_report(validation_df: pd.DataFrame):
    ...
    print(f"  Average depth with comprehensive context: {avg_depth:.2f}")
...
def main():
    ...
    print("WITH COMPREHENSIVE YAML CONTEXT")
    print("=" * 80)
...
def process_full_dataset():
    ...
    logger.info(
        "Starting full dataset processing with enhanced LLM-driven classification..."
    )
    ...
    try:
        ...
        for batch_num in range(total_batches):
            ...
            logger.info(
                f"Processing batch {batch_num + 1}/{total_batches} (rows {start_idx}-{end_idx}) with comprehensive YAML context"
            )
