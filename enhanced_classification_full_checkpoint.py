...
VALIDATION_CSV = (
    "validation_enhanced_fixed_classification_w_tagging_prompt_changes_5.csv"
)
...
CHECKPOINT_DIR = "enhanced_classification_checkpoints"
CHECKPOINT_FREQ = 50  # Save every N items
CHECKPOINT_PREFIX = "enhanced_validation_checkpoint"
FULL_CHECKPOINT_PREFIX = "enhanced_full_checkpoint"
...
class EnhancedTagSystem:
    ...
    def format_compact_prompt(self, max_per_category: int = 15) -> str:
        """FIXED: Include specimen tags in prompt formatting"""
        ...
        specimen_subset = list(sorted(self.specimen_tags))[:max_per_category]  # ADD

        return {
            "technique": technique_subset,
            "research": research_subset,
            "functional": functional_subset,
            "specimen": specimen_subset,  # ADD
        }
...
class EnhancedLLMClassifier:
    ...
    def tag_product(
        ...
        """FIXED: Complete tagging with all 4 categories including specimen tags"""

        logger.info(f"🏷️ TAGGING: '{product_name}'")

        # Get compact tag lists for prompt - INCLUDE SPECIMEN
...
TASK: Assign relevant tags to this product from 4 tag categories.
...
4. Select 0-2 SPECIMEN tags (WHAT species/samples it works with)
5. Only select tags that clearly apply - be selective, not exhaustive
6. Consider BOTH domain classifications when selecting tags
...
SPECIMEN TAGS (select 0-2 most relevant):
{', '.join(tag_categories['specimen'])}
...
{{
    ...
    "tag_confidence": "High/Medium/Low"
        ...
        try:
            ...
            # Validate tags against available tag sets - INCLUDE SPECIMEN
            ...
            total_tags = (
                ...
                + len(validated_result.get("specimen_tags", []))  # ADD SPECIMEN COUNT
            )
    ...
    def _validate_tags(self, result: Dict[str, Any]) -> Dict[str, Any]:
        """FIXED: Validate ALL 4 tag categories including specimen"""

        # Extract and validate all 4 tag categories
        technique_tags = [
            tag
        ...
        # FIX: Add missing specimen tags validation
        specimen_tags = [
            tag
            for tag in result.get("specimen_tags", [])
            if tag in self.tag_system.specimen_tags
        ]
        ...
        return {
            ...
            "specimen_tags": specimen_tags,  # ADD
    ...
    def classify_product(
        ...
        for classification in classifications:
            classification.update(
                {
                    ...
                    "specimen_tags": tagging_result.get("specimen_tags", []),
            ...
            all_tags = (
                ...
                + classification.get("specimen_tags", [])  # ADD SPECIMEN
            )
...
# =============================================================================
# FIX 2: Enhanced CSV Columns Structure
# =============================================================================


def get_enhanced_csv_columns():
    """Streamlined column structure removing high-token reasoning fields"""
    return [
        # Primary classification - CORE DATA ONLY
        "primary_domain",
        "primary_subcategory",
        ...
        "primary_fit_score",
        "primary_path_valid",
        "primary_depth_achieved",
        "validated_path_primary",
        # Secondary classification - CORE DATA ONLY
        "secondary_domain",
        ...
        "total_tags",
        "tag_confidence",
        # Token usage - ESSENTIAL TRACKING ONLY
        "total_token_usage",
...
def test_enhanced_dual_classification():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        specimen_tags = primary.get("specimen_tags", [])  # ADD SPECIMEN
        ...
        if specimen_tags:  # ADD SPECIMEN
            print(f"  Specimen: {', '.join(specimen_tags)}")
...
def find_latest_checkpoint():
    ...
    checkpoint_files = glob.glob(
        os.path.join(CHECKPOINT_DIR, f"{CHECKPOINT_PREFIX}_*.csv")
    )
    ...
    for filepath in checkpoint_files:
        ...
        try:
            # Extract number from filename like "enhanced_validation_checkpoint_150.csv"
...
def find_latest_full_checkpoint():
    """Find the most recent full dataset checkpoint file"""
    if not os.path.exists(CHECKPOINT_DIR):
        return None

    checkpoint_files = glob.glob(
        os.path.join(CHECKPOINT_DIR, f"{FULL_CHECKPOINT_PREFIX}_*.csv")
    )
    if not checkpoint_files:
        return None
    ...
    for filepath in checkpoint_files:
        filename = os.path.basename(filepath)
        try:
            # Extract number from filename like "enhanced_full_checkpoint_1500.csv"
            number = int(filename.split("_")[-1].split(".")[0])
            checkpoint_numbers.append((number, filepath))
        except (ValueError, IndexError):
            continue
...
def process_enhanced_validation_sample():
    """FIXED: Enhanced validation with all columns including specimen tags AND CHECKPOINTING"""
    ...
    try:
        ...
        if latest_checkpoint:
            ...
            validation_df = pd.read_csv(latest_checkpoint)

            # Find the first row with empty classification data to resume from there
            try:
                start_idx = validation_df[
                    validation_df["primary_domain"].isna()
            ...
            validation_df = df.iloc[sample_indices].copy().reset_index(drop=True)
            start_idx = 0
            logger.info(
                f"🆕 No checkpoint found, starting fresh with {sample_size} products"
            )

        # Use complete column structure
        new_columns = get_enhanced_csv_columns()

        # Initialize all new columns if they don't exist
        ...
        logger.info(f"📊 Processing {total_products - start_idx} remaining products...")
        logger.info(
            f"🔍 DataFrame index range: {validation_df.index.min()} to {validation_df.index.max()}"
        )
        logger.info(f"🔍 Loop range: {start_idx} to {total_products - 1}")

        for idx in tqdm(
            range(start_idx, total_products), desc="🔄 Enhanced Dual Classification"
        ):
            try:
                ...
                logger.error(f"❌ KeyError accessing index {idx}: {e}")
                logger.error(f"Available indices: {list(validation_df.index)[:10]}...")
                logger.error(f"DataFrame shape: {validation_df.shape}")
                raise
            ...
            try:
                ...
                # Store enhanced results with all fields
                ...
                if classifications:
                    ...
                    validation_df.at[idx, "primary_depth_achieved"] = primary.get(
                        "depth_achieved", 0
                    )
                    ...
                    # ALL TAG CATEGORIES INCLUDING SPECIMEN
                    ...
                    validation_df.at[idx, "specimen_tags"] = "|".join(
                        primary.get("specimen_tags", [])
                    )  # ADD
                ...
                if len(classifications) > 1:
                    ...
                    validation_df.at[idx, "secondary_domain"] = secondary.get(
                        "domain", ""
                    )
                    ...
                    validation_df.at[idx, "secondary_subsubsubcategory"] = (
                        secondary.get("subsubsubcategory", "")
                    )
                    ...
                    validation_df.at[idx, "secondary_depth_achieved"] = secondary.get(
                        "depth_achieved", 0
                    )
                ...
                validation_df.at[idx, "error_occurred"] = False
                ...
                validation_df.at[idx, "error_occurred"] = True
                validation_df.at[idx, "primary_domain"] = "Other"
                validation_df.at[idx, "primary_subcategory"] = "Error"

            # Save checkpoint every CHECKPOINT_FREQ items or at the end
            if (idx + 1) % CHECKPOINT_FREQ == 0 or idx == total_products - 1:
                checkpoint_file = os.path.join(
                    CHECKPOINT_DIR, f"{CHECKPOINT_PREFIX}_{idx + 1}.csv"
                )
                validation_df.to_csv(checkpoint_file, index=False)
...
def process_full_dataset():
    """Process the full dataset with checkpointing support"""
    ...
    try:
        # Check for existing full dataset checkpoint
        latest_checkpoint = find_latest_full_checkpoint()

        if latest_checkpoint:
            logger.info(f"📁 Found full dataset checkpoint: {latest_checkpoint}")
            full_df = pd.read_csv(latest_checkpoint)

            # Find the first row with empty classification data to resume from there
            try:
                start_idx = full_df[
                    full_df["primary_domain"].isna() | (full_df["primary_domain"] == "")
                ].index[0]
            except IndexError:
                logger.info("Full dataset checkpoint is fully processed. Exiting.")
                generate_enhanced_dual_validation_report(full_df)
                return full_df

            logger.info(
                f"📍 Resuming full dataset from checkpoint at index {start_idx}"
            )
        else:
            ...
            start_idx = 0
            logger.info(
                f"🆕 No full dataset checkpoint found, starting fresh with {len(full_df)} products"
            )
        ...
        logger.info(f"📊 Processing {total_products - start_idx} remaining products...")
        logger.info(
            f"🔍 DataFrame index range: {full_df.index.min()} to {full_df.index.max()}"
        )
        logger.info(f"🔍 Loop range: {start_idx} to {total_products - 1}")

        for idx in tqdm(
            range(start_idx, total_products), desc="🔄 Full Dataset Classification"
        ):
            try:
                name = full_df.at[idx, "Name"]
                description = (
                    full_df.at[idx, "Description"]
                    if "Description" in full_df.columns
                    else ""
                )
            except KeyError as e:
                logger.error(f"❌ KeyError accessing index {idx}: {e}")
                logger.error(f"Available indices: {list(full_df.index)[:10]}...")
                logger.error(f"DataFrame shape: {full_df.shape}")
                raise
            ...
            try:
                ...
                if classifications:
                    primary = classifications[0]
                    full_df.at[idx, "primary_domain"] = primary.get("domain", "")
                    full_df.at[idx, "primary_subcategory"] = primary.get(
                        "subcategory", ""
                    )
                    full_df.at[idx, "primary_subsubcategory"] = primary.get(
                        "subsubcategory", ""
                    )
                    full_df.at[idx, "primary_subsubsubcategory"] = primary.get(
                        "subsubsubcategory", ""
                    )
                    full_df.at[idx, "primary_confidence"] = primary.get(
                        "confidence", ""
                    )
                    full_df.at[idx, "primary_fit_score"] = primary.get(
                        "domain_fit_score", 0
                    )
                    full_df.at[idx, "primary_path_valid"] = primary.get(
                        "is_valid_path", False
                    )
                    full_df.at[idx, "primary_depth_achieved"] = primary.get(
                        "depth_achieved", 0
                    )
                    full_df.at[idx, "validated_path_primary"] = primary.get(
                        "validated_path", ""
                    )

                    # ALL TAG CATEGORIES INCLUDING SPECIMEN
                    full_df.at[idx, "technique_tags"] = "|".join(
                        primary.get("technique_tags", [])
                    )
                    full_df.at[idx, "research_tags"] = "|".join(
                        primary.get("research_tags", [])
                    )
                    full_df.at[idx, "functional_tags"] = "|".join(
                        primary.get("functional_tags", [])
                    )
                    full_df.at[idx, "specimen_tags"] = "|".join(
                        primary.get("specimen_tags", [])
                    )  # ADD
                    full_df.at[idx, "all_tags"] = "|".join(primary.get("all_tags", []))
                    full_df.at[idx, "total_tags"] = primary.get("total_tags", 0)
                    full_df.at[idx, "tag_confidence"] = primary.get(
                        "tag_confidence", ""
                    )
                ...
                if len(classifications) > 1:
                    secondary = classifications[1]
                    full_df.at[idx, "secondary_domain"] = secondary.get("domain", "")
                    full_df.at[idx, "secondary_subcategory"] = secondary.get(
                        "subcategory", ""
                    )
                    full_df.at[idx, "secondary_subsubcategory"] = secondary.get(
                        "subsubcategory", ""
                    )
                    full_df.at[idx, "secondary_subsubsubcategory"] = secondary.get(
                        "subsubsubcategory", ""
                    )
                    full_df.at[idx, "secondary_confidence"] = secondary.get(
                        "confidence", ""
                    )
                    full_df.at[idx, "secondary_fit_score"] = secondary.get(
                        "domain_fit_score", 0
                    )
                    full_df.at[idx, "secondary_path_valid"] = secondary.get(
                        "is_valid_path", False
                    )
                    full_df.at[idx, "secondary_depth_achieved"] = secondary.get(
                        "depth_achieved", 0
                    )
                    full_df.at[idx, "validated_path_secondary"] = secondary.get(
                        "validated_path", ""
                    )

                full_df.at[idx, "error_occurred"] = False

                # Dual function info
                full_df.at[idx, "is_dual_function"] = result.get(
                    "is_dual_function", False
                )

                full_df.at[idx, "classification_count"] = result.get(
                    "classification_count", 1
                )

                # Token usage
                full_df.at[idx, "total_token_usage"] = result.get(
                    "total_token_usage", 0
                )
                full_df.at[idx, "chain_prompting_stages"] = result.get(
                    "chain_prompting_stages", 3
                )

                # Domain selection stage
                stage1_info = result.get("stage1_domain_selection", {})
                full_df.at[idx, "domain_selection_primary_confidence"] = (
                    stage1_info.get("primary_confidence", "")
                )
                full_df.at[idx, "domain_selection_secondary_confidence"] = (
                    stage1_info.get("secondary_confidence", "")
                )

            except Exception as e:
                logger.error(f"❌ Error processing '{name}': {e}")
                full_df.at[idx, "error_occurred"] = True
                full_df.at[idx, "primary_domain"] = "Other"
                full_df.at[idx, "primary_subcategory"] = "Error"

            # Save checkpoint every CHECKPOINT_FREQ items or at the end
            if (idx + 1) % CHECKPOINT_FREQ == 0 or idx == total_products - 1:
                checkpoint_file = os.path.join(
                    CHECKPOINT_DIR, f"{FULL_CHECKPOINT_PREFIX}_{idx + 1}.csv"
                )
                full_df.to_csv(checkpoint_file, index=False)
                logger.info(f"💾 Saved full dataset checkpoint: {checkpoint_file}")

        # Save final result
        full_df.to_csv(OUTPUT_CSV, index=False)
        logger.info(f"✅ Full dataset classification saved to {OUTPUT_CSV}")
...
def generate_enhanced_dual_validation_report(validation_df: pd.DataFrame):
    ...
    # Complete tagging analysis including specimen
    print(f"\n{'COMPLETE TAGGING ANALYSIS (ALL 4 CATEGORIES)':-^60}")
    ...
    # SPECIMEN TAGS ANALYSIS - ADD
    print(f"\n{'TOP SPECIMEN TAGS':-^60}")
    all_specimen_tags = []
    for tags_str in validation_df["specimen_tags"].dropna():
        if tags_str:
            all_specimen_tags.extend(tags_str.split("|"))

    specimen_tagged_products = len(
        validation_df[validation_df["specimen_tags"].str.len() > 0]
    )

    if all_specimen_tags:
        specimen_counts = Counter(all_specimen_tags)
        print(
            f"  Products with specimen tags: {specimen_tagged_products} ({specimen_tagged_products/total_products*100:.1f}%)"
        )
        for tag, count in specimen_counts.most_common(8):
            print(f"  {tag:<35} {count:>5}")
    else:
        print("  No specimen tags found")
...
def main():
    """STREAMLINED: Production-ready main function with full dataset processing"""
    try:
